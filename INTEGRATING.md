...
**Provenance.** Recipe data was compiled from public sources: Berry's books, Trader Vic's guides, Difford's Guide and GitHub-hosted recipe datasets, listed in the README. Only recipe facts were recorded (ingredients, amounts, method, attribution); descriptions and notes are written fresh. The project is MIT-licensed ([LICENSE](LICENSE)); [NOTICE.md](NOTICE.md) covers the imported recipe data and third-party names. The copyright holder's own site needs nothing extra. Anyone else copying the file should keep the MIT notice with it, for example as a LICENSE file next to it.
