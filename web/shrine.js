...
async function main() {
  ...
  // offering answers. Clicking in clears it, so your own words start on an
  // empty line instead of landing inside someone else's; the placeholder
  // keeps an example in view. Only the untouched example goes.
  const opening = $('#prayer').value;
  $('#prayer').addEventListener('focus', e => { if (e.target.value === opening) e.target.value = ''; }, { once: true });
